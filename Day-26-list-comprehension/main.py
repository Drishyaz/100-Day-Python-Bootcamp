...
# YOU ADD 1 TO EACH ITEM IN THE LIST
numbers = [1,2,3]
...
list1 = [int(n.strip()) for n in new_list]      # Then we convert each item into an integer and strip the newline

with open("file2.txt", "r") as f2:
  new_list = f2.readlines()                     # Next we extract the contents of file2.txt
list2 = [int(n.strip()) for n in new_list]      # Then we convert each item into an integer and strip the newline

result = [n for n in list1 if n in list2]       # lastly we check for each item in list1, if present in list2
                                                # Only then do we consider those items
...
# TAKE EACH temp in Celsius AND CONVERT IT INTO temp in Fahrenheit
